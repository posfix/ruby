class Paymentinquiryrequest
    ...
    def execute(req, settings)
        ...
        settings.HashString =
            settings.PrivateKey + req.orderId + settings.Mode +
                settings.transactionDate
        p self.to_xml(req, settings)
        result =
            Core::HttpClient.post(
                settings.BaseUrl + 'rest/payment/inquiry',
    ...
    def to_xml(req, settings)
        return(
            "<?xml version='1.0' encoding='UTF-8' ?><inquiry><orderId>" +
                req.orderId + '</orderId><echo>' + settings.Echo +
                '</echo><mode>' + settings.Mode + '</mode></inquiry>'
        )
    end
end
